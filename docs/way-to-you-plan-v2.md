...
두 도시가 준대척 관계라 MapKit 카메라에서 실제 지표 마커 두 개를 동시에 보여줄 수 없을 때도 도시 좌표를 다른 앞면 지역으로 옮기지 않는다. 실제 도시 좌표와 대권 Route는 그대로 유지하고, 지구 뒤에 가려진 프로필은 화면에 렌더된 Route 점선의 끝에 작은 뒷면 표시로 나타낸다. 사용자가 지구를 움직이는 동안에는 이 표시를 fade out하고, 관성이 끝난 뒤 최종 위치에서 fade in해 경계 떨림을 피한다. 일반 Route에서는 충분히 깊은 뒷면 표시를 숨긴다. 극점 인접 준대척 조합처럼 기존 카메라에서 Route와 양 끝이 화면 밖으로 밀리는 경우에만 별도의 초기 framing을 적용해 경로를 드러내며, 기존에 잘 보이던 도시 조합의 카메라는 유지한다. 실제 도시가 앞면으로 돌아오면 원래 프로필 핀을 다시 표시한다.

홈에는 각 도시의 현지 시각과 현재 날씨를 작게 표시한다. 사용자는 시간 형식·온도 단위와 두 도시 Route 중앙의 Heart 이모지·애니메이션을 설정할 수 있다. 이 정보나 프로필·Signal·배터리 상태가 바뀌어도 사용자가 보던 지구 시점과 도시 Route 좌표는 유지한다.
