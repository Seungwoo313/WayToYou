...
| MapKit 지구본 | 도시 프로필 마커·동적 카메라·근거리 확대·도시 Route·뒷면 마커 완료 | 배송 Route·Gift annotation 미구현 |
...
- annotation 커밋 뒤 2 display frame 후 확대해 전 지구에서 지역으로 들어가는 MapKit 기본 애니메이션을 사용한다. Reduce Motion에서는 애니메이션 없이 같은 최종 카메라를 적용하고, 대기 중 지도 제스처가 시작되면 자동 확대를 취소한다.
